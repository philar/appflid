 ...
 */
int
regexec(struct regexp *prog, char *string)
{
	register char *s;
	struct match_globals g;
	int result=0;
	...
	if (UCHARAT(prog->program) != MAGIC) {
		printk("<3>Regexp: corrupted program result=%d\n",result);
		return result;
	}
...
regtry(struct match_globals *g,struct  regexp *prog, char *string)
{
	register int i;
	register char **sp;
	register char **ep;
...
/*int main(int argc,char **argv){
	...
	printk("hello regex\n");
	if (argc < 3)
		printk("Usage %s pattern test_str\n",argv[0]);
    pattern = malloc(strlen(argv[1])+1);
    test_str = malloc(strlen(argv[2])+1);
    
	memset(pattern,0,strlen(argv[1])+1);
	memset(test_str,0,strlen(argv[2])+1);

	strcpy(pattern,argv[1]);
	strcpy(test_str,argv[2]);
	plen=strlen(pattern);

	reg = regcomp(pattern,&plen);
	if(!reg)
		printk("regcomp faild\n");
	result = regexec(reg,test_str);
